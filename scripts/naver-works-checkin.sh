...
try
    ...
    tell application "Google Chrome"
        ...
        if checkinResult starts with "CLICKED_FIRST" then
            -- 확인 팝업 대기 (폴링)
            set confirmResult to "FAIL: 확인 팝업에서 출근 버튼을 찾지 못했습니다."
            ...
            if confirmResult is "SUCCESS" then
                ...
                if verifyResult is "VERIFIED" then
                    ...
                    set checkinResult to "FAIL_UNVERIFIED: 출근 버튼을 클릭했지만 상태 변경이 확인되지 않았습니다. 네트워크 지연 또는 페이지 응답 지연이 원인일 수 있습니다. 수동으로 출근 상태를 확인해 주세요."
                end if
            else if confirmResult is "WAITING" then
                set checkinResult to "FAIL_UNVERIFIED: 확인 팝업이 표시되지 않았습니다. Chrome 팝업 차단이 원인일 수 있습니다. 수동으로 출근 상태를 확인해 주세요."
            else
